...
import pandas as pd
import scrapy
from crawls.items import WegItem
import urllib
import urllib2
from scrapy.http import FormRequest
import re

out = pd.read_csv("/data/work/virtualenvs/scrapy/crawls/crawls/spiders/data/diff11.csv", sep=',')
...
class Weg(scrapy.Spider):
    ...
    def parse_item1(self, response, meta_row):
        ...
        try:
            search_catagol_number = response.xpath('//*[@id="page"]/div/div/div[3]/div[5]/div/div[3]/div[2]/table/tbody/tr[1]/td[3]/p/text()').extract_first().encode('ascii', 'ignore').split(':')[1]
        except Exception:
            try:
                search_catagol_number = response.xpath('//*[@id="page"]/div/div/div[3]/div[5]/div[2]/div[3]/div[2]/table/tbody/tr[1]/td[3]/a/text()').extract_first()
                ...
                if str(meta_row).strip() in search_catagol_number:
                    ...
                    errback = lambda failure: self.repeat(failure, meta_row)
                    url = 'http://www.weg.net' + response.xpath('//*[@id="page"]/div/div/div[3]/div[5]/div/div[3]/div[2]/table/tbody/tr[1]/td[3]/a/@href').extract_first()
            ...
            if search_catagol_number == str(meta_row).strip():
                callback = lambda response: self.parse_item2(response, meta_row)
                errback = lambda failure: self.repeat(failure, meta_row)
                url = 'http://www.weg.net' + response.xpath('//*[@id="page"]/div/div/div[3]/div[5]/div[2]/div[3]/div[2]/table/tbody/tr[1]/td[3]/a/@href').extract_first()
                return scrapy.Request(url=url, callback=callback, errback=errback)
            else:
                return self.error(meta_row)
