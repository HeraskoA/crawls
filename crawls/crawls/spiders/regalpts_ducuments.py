...
import pandas as pd
import scrapy
from crawls.items import RegalptsDocumentsItem
import urllib
import urllib2
import shutil
...
class Regalpts(scrapy.Spider):
    ...
    def extract_docs(self, response):
        row = response.meta['row']
        pdf = response.xpath('//*[@id="ctl00_Master_ContentPlaceHolder1_ContentPlaceHolderMain_HyperLinkCatalogPage"]/@href').extract_first()
        if pdf:
            name = response.xpath('//*[@id="ctl00_Master_ContentPlaceHolder1_ContentPlaceHolderMain_HyperLinkCatalogPage"]/@title').extract_first()
            yield self.create_item(row, response.urljoin(pdf), name)
        info = response.xpath('//*[@id="ctl00_Master_ContentPlaceHolder1_ContentPlaceHolderMain_HyperLinkInstructionSheet"]/@href').extract_first()
        if info:
            name = response.xpath('//*[@id="ctl00_Master_ContentPlaceHolder1_ContentPlaceHolderMain_HyperLinkInstructionSheet"]/@title').extract_first()
