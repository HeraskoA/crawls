...
from crawls.items import ToshibaItem
...
class ToshibaSpider(CrawlSpider):
    ...
    cookies = {
        ...
        'AMCV_BA23678254F61E630A4C98A5@AdobeOrg':"1099438348|MCIDTS|17395|MCMID|12550951087790814574253316757767919054|MCAAMLH-1503513925|6|MCAAMB-1503519939|NRX38WO0n5BH8Th-nqAG_A|MCOPTOUT-1502922339s|NONE|MCAID|NONE|MCSYNCSOP|411-17402|vVersion|2.1.0",        'Grundfos_cookies_allowed':"true",
    ...
    data = pandas.read_csv("crawls/spiders/data/grundfox_additional_descriptions_work.csv", sep=',')
    ...
    for number in list(data.catalog_number):
    	start_urls.append('http://product-selection.grundfos.com/product-detail.product-detail.html?hits=1&productnumber=' + str(number))
    ...
    def parse(self, response):
        ...
        if response.xpath('//div[@id="imageContainer"]/img/@src').extract()[0] != '':
            item['image'] = 'http://net.grundfos.com/RestServer/imaging/product?productnumber={}&frequency=60&languagecode=RUS&productrange=gmo&searchdomain=SALEABLE&unitsystem=4'.format(number)
        else:
            return
